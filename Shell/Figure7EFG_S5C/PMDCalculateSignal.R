...
EAC_specificPMDs=readDomian("Data/MMSeekR_PMDs/EAC_specificPMDs.bed")
ESCC_specificPMDs=readDomian("Data/MMSeekR_PMDs/ESCC_specificPMDs.bed")
ESCA_sharedPMDs=readDomian("Data/MMSeekR_PMDs/ESCA_sharedPMDs.bed")
ESCA_sharedHMDs=readDomian("Data/MMSeekR_PMDs/ESCA_sharedHMDs.bed")
...
if(nrow(targetFragments)>0){
  ...
  EAC_specificPMDs_result=getDomainCounts(EAC_specificPMDs, "EAC_specificPMDs")
  ESCC_specificPMDs_result=getDomainCounts(ESCC_specificPMDs, "ESCC_specificPMDs")
  ESCA_sharedPMDs_result=getDomainCounts(ESCA_sharedPMDs, "ESCA_sharedPMDs")
  ESCA_sharedHMDs_result=getDomainCounts(ESCA_sharedHMDs, "ESCA_sharedHMDs")

  merge_result=merge(backgroup_result, EAC_specificPMDs_result, by="barcode", all.x=T)
  merge_result=merge(merge_result, ESCC_specificPMDs_result, by="barcode", all.x=T)
  merge_result=merge(merge_result, ESCA_sharedPMDs_result, by="barcode", all.x=T)
  merge_result=merge(merge_result, ESCA_sharedHMDs_result, by="barcode", all.x=T)
  rownames(merge_result)=merge_result$barcode
  merge_result=merge_result[,-1]
  merge_result[is.na(merge_result)]=0
  merge_result$EAC_specificPMDs=merge_result$EAC_specificPMDs/merge_result$total
  merge_result$ESCC_specificPMDs=merge_result$ESCC_specificPMDs/merge_result$total
  merge_result$ESCA_sharedPMDs=merge_result$ESCA_sharedPMDs/merge_result$total
  merge_result$ESCA_sharedHMDs=merge_result$ESCA_sharedHMDs/merge_result$total
  ...
  write.table(merge_result, file=paste0("Data/Figure7EFG_S5C/PMDresult", fragmentIndex, "_result.txt"), row.names = F, col.names = T, quote=F, sep="\t")
}
